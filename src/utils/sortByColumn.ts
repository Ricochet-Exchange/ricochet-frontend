...
export function sortByColumn(
  ...
  switch (column) {
    ...
    case 'superTokenBalanceInUSD':
      if (sortDirection === 'asc') {
        const sorted = [...sortedUpgradeTokensList].sort((a, b) => {
          const aBalance = balances && geckoPriceList ?
            parseFloat(balances[a.superTokenAddress]) *
            parseFloat(
              (geckoPriceList as any)[
                (geckoMapping as any)[a.coin]
              ].usd,
            ) : '0';
          const bBalance = balances && geckoPriceList ?
            parseFloat(balances[b.superTokenAddress]) *
            parseFloat(
              (geckoPriceList as any)[
                (geckoMapping as any)[b.coin]
              ].usd,
            ) : '0';
    ...
    case 'monthlyNetFlow':
      if (sortDirection === 'asc') {
        const sorted = [...sortedUpgradeTokensList].sort((a, b) => {
          // For token a
          const usdPriceStringA = (geckoPriceList as any)[
            (geckoMapping as any)[a.coin]
          ].usd;
          const usdPriceA = new Big(parseFloat(usdPriceStringA));
          let inFlowRateA = 0;
          const inFlowArrayA = flows?.flowsOwned
            ?.filter(
              (flow: Flow) =>
                flow.token.id === a.superTokenAddress.toLowerCase(),
            );
          ...
          let outFlowRateA = 0;
          const outFlowArrayA = flows?.flowsReceived
            ?.filter(
              (flow: Flow) =>
                flow.token.id === a.superTokenAddress.toLowerCase(),
            );
          ...
          // For token b
          const usdPriceStringB = (geckoPriceList as any)[
            (geckoMapping as any)[b.coin]
          ].usd;
          const usdPriceB = new Big(parseFloat(usdPriceStringB));
          let inFlowRateB = 0;
          const inFlowArrayB = flows?.flowsOwned
            ?.filter(
              (flow: Flow) =>
                flow.token.id === b.superTokenAddress.toLowerCase(),
            );
          ...
          let outFlowRateB = 0;
          const outFlowArrayB = flows?.flowsReceived
            ?.filter(
              (flow: Flow) =>
                flow.token.id === b.superTokenAddress.toLowerCase(),
            );
