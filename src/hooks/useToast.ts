...
import { hideToast, showErrorToast, showSuccessToast, showToast } from 'components/common/Toaster';
...
export const useToasts = (hideOnUnmount = true) => {
	...
	const showToastWrapped = useCallback(
		(content: ToastContent, type: TypeOptions = 'info', options?: ToastOptions) => {
			hideToastWrapped();
	...
	useEffect(() => {
		...
		hideToastWrapped();
	}, [hideOnUnmount, hideToastWrapped]);
