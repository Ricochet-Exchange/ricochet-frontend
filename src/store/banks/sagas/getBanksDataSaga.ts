...
export function* getBanksDataSaga() {
	try {
		...
		const bankAddresses: ReturnType<typeof selectBanksAddresses> = yield select(selectBanksAddresses);
		...
		const banks: BankType[] = yield all(
			bankAddresses.map((bankAddress: string) => call(getBankData, bankAddress, accountAddress, web3)),
		);
