...
export const startFlowAction = (payload: string, config: ConfigType, callback: (e?: string) => void) => ({
	type: MainActionTypes.START_FLOW,
	payload: { amount: payload, config, callback },
});

export const stopFlowAction = (config: ConfigType, callback: (e?: string) => void) => ({
