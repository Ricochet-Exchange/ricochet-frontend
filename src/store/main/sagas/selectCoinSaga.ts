...
import {
  mainSetState, selectDowngradeCoin, selectSwapCoin, selectUpgradeCoin, showTokenList, 
} from '../actionCreators';
...
export function* selectSwapCoinSaga({ payload }: ReturnType<typeof selectSwapCoin>) {
  try {
    yield put(mainSetState({ selectedSwapCoin: payload.selectedSwapCoin }));
    yield put(modalHide());
  } catch (e) {
    transformError(e);
  }
