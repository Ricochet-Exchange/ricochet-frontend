...
// Rewards APY
const rewardsUrl =
  'https://api.thegraph.com/subgraphs/name/sushiswap/matic-minichef'; // https://thegraph.com/hosted-service/subgraph/sushiswap/matic-minichef
...
// Fees APY
const feesUrl =
  'https://api.thegraph.com/subgraphs/name/sushiswap/matic-exchange'; // https://thegraph.com/hosted-service/subgraph/sushiswap/matic-exchange
...
const coingeckoUrl =
  'https://api.coingecko.com/api/v3/simple/price?ids=matic-network%2Csushi&vs_currencies=usd';

const retrieveAPY = async (
  ...
  const rewardsResp = await axios.post(rewardsUrl, {
    query: rewardsQuery(addr, pid),
  });
  const pool = rewardsResp.data.data.pools[0];
  const maticRewardsPerSecond =
    (pool.rewarder.rewardPerSecond * pool.allocPoint) /
    pool.miniChef.totalAllocPoint;
  const sushiRewardsPerSecond =
    (pool.miniChef.sushiPerSecond * pool.allocPoint) /
    pool.miniChef.totalAllocPoint;
  ...
  const rewardsUsdPoolPerYear =
    365.25 *
    (maticRewardsPerDay * maticPrice + sushiRewardsPerDay * sushiPrice);

  const rewardsApy =
    (rewardsUsdPoolPerYear / parseFloat(pair.dayData[0].reserveUSD)) * 100;
    ...
    <div className={styles.balance_container}>
      ...
      >
        <span className={styles.apyTooltip_span}>
          {`Fees: ${trimPad(feesApy, 2)}% / Rewards: ${trimPad(
            rewardsApy,
            2
          )}%`}
        </span>
