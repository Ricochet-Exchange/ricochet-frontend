...
const retrieveAPY = async (
	...
	lastweekFees = lastweekFees.reduce((acc: number, el: number) => (el && el !== Infinity ? acc + el : acc), 0);
	...
	const maticRewardsPerSecond = (pool.rewarder.rewardPerSecond * pool.allocPoint) / pool.miniChef.totalAllocPoint;
	const sushiRewardsPerSecond = (pool.miniChef.sushiPerSecond * pool.allocPoint) / pool.miniChef.totalAllocPoint;
	...
	const rewardsUsdPoolPerYear = 365.25 * (maticRewardsPerDay * maticPrice + sushiRewardsPerDay * sushiPrice);
...
export default function LpApy(props: Props) {
	...
	return (
		<div className={styles.balance_container}>
			<span {...props} className={styles.balance} data-tip data-for="apyTooltip">{`APY: ${trimPad(
				apy,
				2,
			)} %`}</span>
			<ReactTooltip id="apyTooltip" place="right" effect="solid" className={styles.apyTooltip}>
