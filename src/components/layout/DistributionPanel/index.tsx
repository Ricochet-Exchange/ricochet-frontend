...
export const DistributionPanel: FC<IProps> = ({
  distribution,
  coingeckoPrice = 0,
  isLoading,
}) => {
  ...
  const getSubscribeLinkComponent = (
  ...
  const getUnsubscribeLinkComponent = (
    _distribution: Distribution,
    isMobile: boolean
  ) => (
    <div
      className={
        isMobile ? styles.unsubscribedSmallDevice : styles.unsubscribed
      }
      tabIndex={0}
      role="button"
      onClick={() =>
              ...
              <div className={styles.columns}>
                  ...
                  {isLoading ? (
                    ...
                    <div>
                      <div className={styles.column}>
                        ...
                        <span className={styles.externalLink}>
                          <AddressLink
                            ...
                            addressLink={
                              distribution?.publisher &&
                              getAddressLink(distribution?.publisher)
                            }
                          />
                        </span>
                      </div>
                      {!distribution?.approved
                        ? distribution &&
                          getSubscribeLinkComponent(distribution, false)
                        : distribution &&
                          getUnsubscribeLinkComponent(distribution, false)}
                    </div>
                  )}
                </div>
                <div className={styles.column}>
                  ...
                  {isLoading ? (
                    ...
                    <>
                      <Image
                        src={iconsCoin[distribution?.symbol as Coin]!}
                ...
                <div className={styles.column}>
                  ...
                  {isLoading ? (
                    ...
                    <span className={styles.contract}>
                      {distribution &&
                        format(
                          new Date(distribution?.createdAtTimestamp * 1000),
                          'dd MMM yyyy'
                        )}
                    </span>
                  )}
                ...
                <div className={styles.column}>
                  ...
                  {isLoading || !distribution ? (
                    ...
                    <span className={styles.contract}>
                      <ReactTimeAgo
                        date={new Date(distribution?.updatedAtTimestamp * 1000)}
                      />
                    </span>
                  )}
                </div>
                ...
                {!isLoading && (
                  <div className={styles.columnMobile}>
                    <span className={styles.contract}>
                      {!distribution?.approved
                        ? distribution &&
                          getSubscribeLinkComponent(distribution, true)
                        : distribution &&
                          getUnsubscribeLinkComponent(distribution, true)}
                    </span>
                  </div>
                )}
              </div>
