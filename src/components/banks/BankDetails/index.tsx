...
export const BankDetails: FC<Props> = ({ bank, accountAddress, handleSignIn }) => {
	...
	return (
		<>
			<tr className={styles.bankDetails}>
				...
				<td>
					<div className={styles.priceWithIcon}>
						<img src={iconsCoin[bank.debtToken.symbol as Coin]} alt={bank.debtToken.symbol} />
						<h3>{formatter(+bank.reserveBalance / 1e18, 2)}</h3>
					</div>
				</td>
				<td>
					<div className={styles.priceWithIcon}>
						<img src={iconsCoin[bank.collateralToken.symbol as Coin]} alt={bank.collateralToken.symbol} />
				...
				<td>
					{accountAddress ? (
						<>
							{!bank.vault.hasVault ? (
								<Button label="" className={styles.button} onClick={handleVisionModal}>
			...
			<ModalCreateVaultContainer bank={bank} onCloseModal={handleOnCloseModal} visibleModal={visibleModal} />
		</>
	);
};
