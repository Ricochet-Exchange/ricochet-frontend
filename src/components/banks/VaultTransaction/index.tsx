...
export const VaultTransaction: FC<Props> = ({
	...
	const needsRepayUnlock = () => {
		...
		const needsApproval = +bank.debtToken.unlockedAmount === 0 || +value > +bank.debtToken.unlockedAmount;
		return isRepay && needsApproval && !localApproved;
	};

	const { t } = useTranslation();

	const isCollateral = activeTransaction === 'withdraw' || activeTransaction === 'deposit';

	const vaultTxCalcValues = getVaultTxCalcValues(bank, activeTransaction, value);
				...
				<>
					<div className={styles.VaultTransaction__preview}>
						...
						<div className={styles.VaultDetail}>
							...
							<h3>
								{vaultTxCalcValues.newLiquidationPrice.toFixed(4)} {bank.collateralToken.symbol}
								/USD
							</h3>
						</div>
					</div>
					<div className={styles.VaultTransaction__form__Total}>
						<div className={styles.VaultTransaction__form}>
							{needsRepayUnlock() ? (
								<>
									...
									<ApproveToken isLoadingApprove={isLoadingApprove} onApproveClick={onApproveClick} />
								</>
							) : (
								<>
									<p>
										{t('How much')}{' '}
										{isCollateral ? bank.collateralToken.symbol : bank.debtToken.symbol}{' '}
						...
						<div className={styles.VaultTransaction__buttons}>
							...
							<Button label={t('Cancel')} onClick={onCancel} className={styles.linkButton} />
							<Button
