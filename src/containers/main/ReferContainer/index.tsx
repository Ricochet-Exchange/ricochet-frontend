...
export const ReferContainer: React.FC<IProps> = () => {
  ...
  const validations = (input: string) => {
    const criteria: [boolean, string][] = [
      [
        new Blob([input]).size <= 32,
        t('Cannot be larger than 32 characters (Some characters count as 2)'),
      ],
  ...
  const filterValidationErrors = (
    validationResults: [boolean, string][]
  ): string[] =>
  ...
  useEffect(() => {
    if (address && contract) {
      contract.methods
        .addressToAffiliate(address.toLowerCase())
        .call()
  ...
  const handleReferralId = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    setValidationErrors(filterValidationErrors(validationResults));
  };
  ...
  const handleCopy = () => {
    navigator.clipboard
      .writeText(`${AFFILIATE_URL_PREFIX}${currentReferralId}`)
      .catch();
  };
