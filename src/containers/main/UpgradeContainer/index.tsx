...
export const UpgradeContainer: FC<IProps> = ({ address, balance }) => {
	...
	const coingeckoUrl =
		'https://api.coingecko.com/api/v3/simple/price?ids=richochet%2Cusd-coin%2Cdai%2Cmaker%2Cethereum%2Cwrapped-bitcoin%2Cidle%2Cmatic-network%2Csushi%2Cstake-dao&vs_currencies=usd';
	const geckoMapping = {
		USDC: 'usd-coin',
		MATIC: 'matic-network',
		ETH: 'ethereum',
	...
	useEffect(() => {
		axios.get(coingeckoUrl).then((response) => {
			// TODO: Get the price of sdam3CRV
			response.data.sdam3CRV = { usd: 0 };
						...
						sortedUpgradeTokensList.map((token, index) => {
							...
							return (
								...
								>
									...
									<td className={styles.section}>
										{token && balances ? (
											balances && parseFloat(balances[token.superTokenAddress]).toFixed(8)
										) : (
											<span className={styles.wallet_loading}>
												<Skeleton height={30} width={60} />
											</span>
