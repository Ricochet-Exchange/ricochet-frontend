...
export const InvestContainer: React.FC<IProps> = ({ path }) => {
  ...
  const { address, balances, isLoading, coingeckoPrices } = state;
  ...
  const handleStart = useCallback(
    (config: { [key: string]: string }) =>
      (amount: string, callback: (e?: string) => void) => {
  ...
  function retrieveEndDate(
    ...
    const sameCoinAFlows = flowConfig.filter(
      (flow_) => flow_.coinA === flow?.coinA
    );
    const outgoing = sameCoinAFlows.map(
      (flow_) => currentState[flow_.flowKey]?.placeholder || '0'
    );
    const outgoingSum = outgoing.reduce(sumStrings, 0);
    const bal = parseFloat(
      (currentBalances && currentBalances[flow?.tokenA || '']) || '0'
    );
  ...
  function computeStreamEnds(currentState: any, currentBalances: any) {
    ...
    Object.values(FlowEnum).forEach((flowEnum: FlowEnum) => {
      streamEnds[flowEnum] = retrieveEndDate(
        flowEnum,
        currentState,
        currentBalances
      );
        ...
        <div className={styles.content}>
          {filteredList.map((element, idx) => (
            <div
              ...
              key={`${element.coinA}-${element.coinB}-${element.flowKey}`}
            >
              <PanelChange
                ...
                coinA={element.coinA}
                coingeckoPrice={
                  coingeckoPrices ? coingeckoPrices[element.tokenA] : 0
                }
                coinB={element.coinB}
                ...
                subsidyRate={
                  new Date()
                    ...
                    .reverse()
                    .join('') >=
                  (state[element.flowKey]?.subsidyRate.endDate
                    .split('/')
                    .reverse()
                    .join('') || '0')
                    ? undefined
                ...
                exchangeKey={
                  element.flowKey.replace('FlowQuery', '') as ExchangeKeys
                }
                indexVal={idx}
