...
export const DistributionContainer: React.FC<IProps> = () => {
  ...
  const { address, balances, isLoading, coingeckoPrices } = mainState;

  const distributionsState = useShallowSelector(selectDistributions);
  const { distributions, isLoading: isDistributionLoading } =
    distributionsState;
  ...
  const filteredList = distributions.filter((distribution) =>
    distribution.id.includes(search.toLowerCase())
  );
  return (
    <div className={styles.outer_container}>
      ...
      <div className={styles.container}>
        ...
        <div className={styles.content}>
          ...
          {!isLoading &&
            filteredList.map((distribution) => (
              <div className={styles.panel} key={`${distribution.id}`}>
                <DistributionPanel
                  distribution={distribution}
                  coingeckoPrice={
                    coingeckoPrices && coingeckoPrices[distribution.token]
                  }
                  isLoading={isDistributionLoading}
                />
              </div>
